...
def execute(cmd, output_file="", log=True):
  processes = list()
  if log:
...
def git_apply(result, cmd, patch_file, output_dir):
  ...
  rc = execute("%s %s 1>%s 2>&1" % (cmd, patch_file, output_file), output_file)
  output = ""
...
def mvn_clean(result, mvn_repo, output_dir, mvn_profile):
  ...
  rc = execute("mvn clean -Dmaven.repo.local=%s %s 1>%s 2>&1" % (mvn_repo, mvn_profile, output_file), output_file)
  if rc != 0:
...
def mvn_install(result, mvn_repo, output_dir, mvn_profile):
  ...
  rc = execute("mvn install -U -DskipTests -Dmaven.repo.local=%s %s 1>%s 2>&1" % (mvn_repo, mvn_profile, output_file), output_file)
  if rc != 0:
...
def mvn_test(result, mvn_repo, output_dir, mvn_profile):
  ...
  rc = execute("mvn verify --fail-at-end -Dmaven.repo.local=%s %s 1>%s 2>&1" % (mvn_repo, mvn_profile, output_file), output_file)
  if rc == 0:
